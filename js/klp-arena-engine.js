(() => {
    ...
    function playerLabel(id) {
        ...
        const custom = humanNames[id + 1];
        return (p && !p.bot && custom) ? custom : names[id];
    }
    ...
    if (!room) {
        try {
            room = localStorage.getItem(hostRoomStorageKey) || null;
        } catch (e) { /* ignore */
        }
    }
    ...
    function loadTotalScores() {
        try {
            ...
            if (Array.isArray(parsed)) return {
                scores: starts.map((_, i) => Number(parsed[i]) || 0),
                names: starts.map((_, i) => names[i])
            };
    ...
    function saveTotalScores() {
        try {
            localStorage.setItem(TOTAL_SCORE_KEY, JSON.stringify({scores: totalScores, names: scoreNames}));
    ...
    function reset() {
        ...
        if (!continuous) {
            scores.fill(0);
            scoreNames.forEach((_, i) => {
                scoreNames[i] = playerLabel(i);
            });
        }
        updatePanel();
    }
    ...
    function homeSpawn(id) {
        return homeSpawns[id] || [Math.floor(cols / 2), Math.floor(rows / 2)];
    }
    ...
    // Base-ward portion of an exposed rival's trail (closest to their own territory)
    // rather than the freshest segment right next to their head — sends bots to
    // intercept the abandoned line from a distance, so the kill genuinely comes from
    // crossing the trail rather than from touching the opponent. Real human players
    // (the people who actually joined via QR code) are always the preferred target
    // over another bot when both are exposed and in range — bots specifically hunt
    // people, not just whoever's nearest, so the match stays a real challenge for the
    // humans playing instead of bots mostly fighting each other.
    function huntTarget(p) {
        ...
        if (p.bot && bestHuman) return bestHuman;
    ...
    // their land — once that rival is genuinely nearby. Human rivals use the wider
    // BOT_HUMAN_ATTACK_RANGE so bots notice (and go after) real players from much further
    // away than they'd ever bother reacting to another bot.
    ...
    function moveBot(p, dt, now) {
        ...
        if (!target && Math.random() < 0.05) {
            ...
            const candidates = [];
            for (let y = 1; y < rows - 1; y++) for (let x = 1; x < cols - 1; x++) if (ownerGrid[y][x] === null && grid[y][x] === 0) candidates.push({
                x,
                y
            });
        ...
        if (target) {
            ...
            if (!cur || reached || stale) {
                p.aiExpandTarget = botExpansionTarget(p) || frontierTarget(p) || {
                    x: Math.floor(cols / 2),
                    y: Math.floor(rows / 2)
                };
            }
        ...
        if (!moved) {
            ...
            for (const avoidSelf of [true, false]) {
                ...
                for (const [mx, my] of tries) {
                    ...
                    if (mx !== 0 && !blocked(tx, p.y, p.id) && (!avoidSelf || !wouldSelfCross(p, Math.floor(tx), Math.floor(p.y)))) {
                        p.x = tx;
                        moved = true;
                        p.aiDx = mx;
                        p.aiDy = 0;
                        p.heading = Math.atan2(0, mx);
                        break;
                    }
                    if (my !== 0 && !blocked(p.x, ty, p.id) && (!avoidSelf || !wouldSelfCross(p, Math.floor(p.x), Math.floor(ty)))) {
                        p.y = ty;
                        moved = true;
                        p.aiDx = 0;
                        p.aiDy = my;
                        p.heading = Math.atan2(my, 0);
                        break;
                    }
                }
            }
            if (!moved) {
                ...
                for (const [sx, sy] of spread) {
                    const tx = p.x + sx;
                    const ty = p.y + sy;
                    if (!blocked(tx, ty, p.id)) {
                        p.x = tx;
                        p.y = ty;
                        moved = true;
                        p.aiDx = Math.sign(sx);
                        p.aiDy = Math.sign(sy);
                        p.heading = Math.atan2(sy, sx);
                        break;
                    }
                }
            }
            if (!moved && Math.random() < 0.25) {
                ...
                if (!blocked(tx, p.y, p.id)) {
                    p.x = tx;
                    p.aiDx = fx;
                    p.aiDy = 0;
                    p.heading = Math.atan2(0, fx);
                }
                if (!blocked(p.x, ty, p.id)) {
                    p.y = ty;
                    p.aiDx = 0;
                    p.aiDy = fy;
                    p.heading = Math.atan2(fy, 0);
                }
            }
        }
        sweepEnterCell(p, now);
    ...
    function captureTerritory(p) {
        ...
        const stack = [];
        const tryPush = (x, y) => {
            if (x >= 0 && y >= 0 && x < cols && y < rows && !outside[y][x] && !isWall(x, y)) {
                outside[y][x] = true;
                stack.push([x, y]);
            }
        };
    ...
    function killPlayer(p, now, killer) {
        ...
        if (continuous) {
            ...
            p.bot = true;
            setTimeout(() => {
                if (players[p.id] === p) respawnPlayer(p);
            }, 1500);
        }
    }
    ...
    function tick(dt, now) {
        ...
        for (const k in remoteInputs) {
            if (now - remoteInputs[k].t > 2500) delete remoteInputs[k];
        }
        players.forEach(p => {
            if (p.bot) moveBot(p, dt, now); else movePlayer(p, dt, now);
        });
        for (let i = boosts.length - 1; i >= 0; i--) {
            if (now > boosts[i].expiresAt) boosts.splice(i, 1);
        }
        if (now >= boostsEnabledAt && boosts.length < 4 && Math.random() < 0.018) spawnBoost();
        if (!freezeSpawned && now >= freezeSpawnAt) {
            spawnFreeze();
            freezeSpawned = true;
        }
    ...
    function drawFish(ctx, p, x, y, now) {
        ...
        let mouthOpen = 0;
        if (p.boostStartAt) {
            const elapsed = now - p.boostStartAt;
            if (elapsed >= 0 && elapsed < 400) mouthOpen = Math.sin((elapsed / 400) * Math.PI);
        }
        ctx.strokeStyle = '#18302c';
        ctx.lineWidth = 1.4;
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.arc(15, 4, 3, 0.2, Math.PI - 0.2);
        ctx.stroke();
        if (mouthOpen > 0.05) {
            ctx.fillStyle = '#18302c';
            ctx.beginPath();
            ctx.ellipse(15, 5, 3.2, 2 + mouthOpen * 3.5, 0, 0, Math.PI * 2);
            ctx.fill();
        }
        if (p.speedBoostUntil && performance.now() < p.speedBoostUntil) {
            ctx.save();
            ctx.translate(12, -14);
            ctx.fillStyle = '#111';
    ...
    function drawDangerIndicator(now) {
        ...
        for (const other of players) {
            ...
            for (const c of other.trail) {
                ...
                if (d < minDist) {
                    minDist = d;
                    threatAngle = Math.atan2((c.y + .5) - focus.y, (c.x + .5) - focus.x);
                }
            }
        }
        const threshold = 4.5;
        ...
        if (!dangerWasClose && intensity > 0.2) {
            playDangerSound();
            dangerWasClose = true;
        }
    ...
    function draw() {
        ...
        if (isFullscreen) {
            ...
            players.forEach((p, i) => {
                ...
                ctx.fillStyle = colors[p.id] || '#888';
                ctx.beginPath();
                ctx.arc(panelX + 18, rowY + rowH / 2, 7, 0, Math.PI * 2);
                ctx.fill();
                ctx.strokeStyle = '#000';
                ctx.lineWidth = 1.5;
                ctx.stroke();
        ...
        if (roundOver) {
            ctx.fillStyle = 'rgba(24,48,44,.78)';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            const counts = new Array(players.length).fill(0);
            let total = 0;
            for (let y = 0; y < rows; y++) for (let x = 0; x < cols; x++) {
            ...
            ctx.fillText(players[winnerIndex] ? playerLabel(winnerIndex) + ' VINNER' : 'UAVGJORT', canvas.width / 2, canvas.height / 2 + 12);
        }
    }
    ...
    document.getElementById('restart').addEventListener('click', (e) => {
        ...
        if (networkSocket?.readyState === WebSocket.OPEN) {
            networkSocket.send(JSON.stringify({type: 'restart'}));
        } else {
            round++;
            reset();
        }
    ...
    document.getElementById('back')?.addEventListener('click', (e) => {
        e.currentTarget.blur();
        try {
            localStorage.removeItem('klp-fiske-host-room');
        } catch (e) {
        }
        if (networkSocket?.readyState === WebSocket.OPEN) {
            try {
                networkSocket.send(JSON.stringify({type: 'close-room'}));
            } catch (e) {
            }
    ...
    function connectNetwork() {
        ...
        socket.addEventListener('open', () => {
            ...
            socket.send(JSON.stringify(roomCode ? {
                type: 'host-reconnect',
                room: roomCode
            } : (room ? {type: 'host-reconnect', room} : {type: 'create'})));
        });
        ...
        socket.addEventListener('message', event => {
            ...
            if (message.type === 'error') {
                ...
                roomCode = null;
                try {
                    localStorage.removeItem(hostRoomStorageKey);
                } catch (e) { /* ignore */
                }
            ...
            if (message.type === 'created') {
                ...
                try {
                    localStorage.setItem(hostRoomStorageKey, roomCode);
                } catch (e) { /* ignore */
                }
                continuous = true;
                updateJoinPanel();
            ...
            if (message.type === 'players') {
                message.players.forEach(p => {
                    humanNames[Number(p.id.slice(1))] = p.name;
                });
            ...
            if (message.type === 'remote-input') {
                if (Number.isFinite(message.input.x) && Number.isFinite(message.input.y)) remoteInputs[message.id.slice(1)] = {
                    x: message.input.x,
                    y: message.input.y,
                    t: performance.now()
                };
            }
            if (message.type === 'freeze-pick') {
    ...
    const fsExitEl = document.getElementById('fullscreenExit');
    const fsRootEl = document.getElementById('arenaRoot') || document.getElementById('game');
    if (fsBtnEl) fsBtnEl.addEventListener('click', (e) => {
        e.currentTarget.blur();
        if (fsRootEl.requestFullscreen) fsRootEl.requestFullscreen(); else if (fsRootEl.webkitRequestFullscreen) fsRootEl.webkitRequestFullscreen();
    });
    if (fsExitEl) fsExitEl.addEventListener('click', (e) => {
        e.currentTarget.blur();
        if (document.exitFullscreen) document.exitFullscreen(); else if (document.webkitExitFullscreen) document.webkitExitFullscreen();
    });
    document.addEventListener('fullscreenchange', () => {
        if (document.fullscreenElement) fsExitEl.style.display = 'block'; else fsExitEl.style.display = 'none';
    });
